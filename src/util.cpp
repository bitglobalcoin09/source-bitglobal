...
#else
    const char* pszModule = "Bitglobalcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitglobalcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitglobalcoin
    // Mac: ~/Library/Application Support/Bitglobalcoin
    // Unix: ~/.Bitglobalcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitglobalcoinData";
#else
    fs::path pathRet;
    ...
    return pathRet / "BitglobalcoinData";
#else
    // Unix
    return pathRet / ".BitglobalcoinData";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Bitglobalcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Bitglobalcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Bitglobalcoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("Bitglobalcoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
