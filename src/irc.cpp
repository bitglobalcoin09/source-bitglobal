...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("Bitglobalcoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BitglobalcoinTEST2\r");
            Send(hSocket, "WHO #BitglobalcoinTEST2\r");
        } else {
            // randomly join #Bitglobalcoin00-#Bitglobalcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Bitglobalcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Bitglobalcoin%02d\r", channel_number).c_str());
        }
